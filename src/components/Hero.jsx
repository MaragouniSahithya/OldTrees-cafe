...
// 3D Leaf Particle Component
const Leaves = ({ count = 50 }) => {
  const mesh = useRef();
  const dummy = useMemo(() => new THREE.Object3D(), []);
  
  // Generating initial random positions and properties for leaves
  ...
  return (
    <section id="home" className="relative h-screen w-full overflow-hidden bg-dark flex items-center justify-center">
      ...
      <div 
        ...
        style={{ 
          backgroundImage: 'url("https://images.unsplash.com/photo-1554118811-1e0d58224f24?q=80&w=2047&auto=format&fit=crop")', // Beautiful outdoor garden cafe aesthetic
        }}
      />
      ...
      <div className="relative z-30 container mx-auto px-6 text-center max-w-4xl pt-20">
        ...
        >
          <button 
            ...
            className="w-full sm:w-auto glass-button flex items-center justify-center gap-2"
          >
            <span>🍽️</span> Reserve a Table
          </button>
          <button 
            onClick={() => handleScrollTo('#menu')}
            className="w-full sm:w-auto glass-button-outline flex items-center justify-center gap-2"
          >
            <span>📋</span> View Menu
          </button>
        </motion.div>
